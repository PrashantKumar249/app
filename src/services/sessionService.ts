...
export async function getFirstUnattemptedQuestion(
  ...
  const latest = await getLatestAttemptsForTopic(topicId)
  for (let q = 1; q <= totalQuestions; q++) {
    if (!latest.has(q)) return q
  }
  return 1
}
...
export async function startSession(topicId: string): Promise<Session> {
  ...
  const startQuestion = await getFirstUnattemptedQuestion(topicId, topic.totalQuestions)
  ...
  const session: Session = {
    ...
    lastSequentialQuestion: startQuestion,
    totalTimeSpent: 0,
    status: 'active',
    navigationMode: 'sequential',
    activeQuestionNumber: startQuestion,
...
export async function submitAnswer(
  ...
  const canAdvance = nextQuestion <= topic.totalQuestions

  const updated: Session = {
    ...session,
    totalTimeSpent: session.totalTimeSpent + timeTakenMs,
    currentQuestion: canAdvance ? nextQuestion : questionNumber,
    lastSequentialQuestion: canAdvance
      ? nextQuestion
      : Math.max(session.lastSequentialQuestion, questionNumber),
    navigationMode: 'sequential',
